...
const uint64_t MASK = 0xe0000000;

uint32_t Hashtable::get_frequency(uint64_t hash){
  if(offsets[hash] >= MASK) {
    // Need to look into 2nd level hash
    uint32_t l2index = offsets[hash] & ~MASK;
    return (l2hashtable->l2_get_offset(l2index + 1, 0, 0, 0) - l2hashtable->l2_get_offset(l2index, 0, 0, 0))/36;
  }
  uint32_t index = 1;
  while((offsets[hash + index] & MASK) == MASK)
    index++;
  return offsets[hash + index] - offsets[hash];
}
...
void Hashtable::write_offsets_to_file(const char * name){
  ...
  data = fwrite(offsets, sizeof(uint32_t), offsets_size, f);
...
std::vector<uint32_t> Hashtable::construct_table2(Hashtable *hashtable, uint32_t total_size, std::vector<uint32_t> ** &table, uint32_t l2_threshold) {
  ...
  for(i = 0; i < table_size; i++){
    ...
    if (l2_threshold && frequency >= l2_threshold) {
      buckets.push_back(i);
      hashtable->set_offset(i, l2_index | MASK);
      l2_index++;
      continue;
    }
    hashtable->set_offset(i, total_size);
