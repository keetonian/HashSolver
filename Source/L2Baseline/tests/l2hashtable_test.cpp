...
TEST(l2hashtable_test, creates_empty_hashtable) {
  L2Hashtable l2hashtable = L2Hashtable(0);
  EXPECT_EQ(l2hashtable.l2_get_locations_size(), 0);
  EXPECT_EQ(l2hashtable.l2_get_num_tables(), 0);
}

TEST(l2hashtable_test, pearson_hash) {
  L2Hashtable l2hashtable = L2Hashtable(0);
  for (int i = 0; i <= 255; i++) {
    char c = (char)i;
    EXPECT_TRUE(l2hashtable.pearsonHash(&c) < 256);
  }
}

TEST(l2hashtable_test, get_index) {
  L2Hashtable l2hashtable = L2Hashtable(0);
  EXPECT_EQ(0, l2hashtable.l2_get_index(0, 0, 0, 0));
  EXPECT_EQ(10, l2hashtable.l2_get_index(0, 0, 0, 10));
  EXPECT_EQ((1<<8), l2hashtable.l2_get_index(0, 0, 1, 0));
  EXPECT_EQ((6<<8), l2hashtable.l2_get_index(0, 1, 0, 0));
  EXPECT_EQ((36<<8), l2hashtable.l2_get_index(1, 0, 0, 0));
}
/*
TEST(l2hashtable_test, creates_empty_hashtable_for_size) {
